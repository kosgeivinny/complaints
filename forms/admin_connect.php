...
if (isset($_POST["submit"]))
{
    ...
    if (empty($firstname) || empty($lastname) || empty($surname) || empty($gender) || empty($email) || empty($username) || empty($password)) {
        ?>
        <script> alert ('Oops!! cant leave any field blank');
            window.location.assign('admin_signup.php');
        </script>
    <?php
    }
    else {
    ...
    if(move_uploaded_file($_FILES['image']['tmp_name'], $directory. $image)){
        $sql="INSERT INTO admin(Image,First_name,Last_name,Surname,Gender,Email,Username,Password)".
            "VALUES('$upl','$firstname','$lastname','$surname','$gender','$email','$username','$password')";
    }
