...
$name=$_SESSION['name'];
    ...
    $rus = "SELECT First_name, Department, Complain FROM staff_complain WHERE staff_complain.First_name='$name'";
    $sur = $conn->Query($rus);
     
    echo "<table class='table table-bordered' cellspacing='15px' border='0px' style='width:980px;height:auto;'>
    <thead> 
    <tr class='info'><th>Department</th><th>Complain</th></tr></thead>";
    
    while($row =mysqli_fetch_assoc($sur)) {

        echo "<tr><td>".$row["Department"]."</td><td>".$row["Complain"]."</td>";
