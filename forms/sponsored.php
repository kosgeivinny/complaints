    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper" >
            <div id="page-inner">
                <div class="row">
                    <div class="col-md-12">
                     <h2 align="center"> SPONSORED CHILDREN </h2>   
                    </div>
                </div>              
                 <!-- /. ROW  -->
                  <hr />
...
$homeid=$_SESSION['id'];
    ...
    $rus = "SELECT child.Image,child.First_name,child.Last_name,child.Admission_number,sponsor.Firstname,sponsor.Lastname,sponsor.Gender,sponsor.Age,sponsor.Status,sponsor.Duration,sponsor.Phone,sponsor.Email FROM child INNER JOIN sponsor WHERE child.Home_ID='$homeid' AND child.Admission_number=sponsor.Admission_number   ";
    $sur = $conn->Query($rus);
     
     echo "<table class='table table-bordered' cellspacing='15px' border='0px' style='width:980px;height:auto;'>
    <thead> 
    <tr class='success'><th></th><th>First Name</th><th>Last Name</th><th>ADM_NO</th><th></th></tr></thead>";
    ...
    while($row =mysqli_fetch_assoc($sur)) {
        ...
        echo "<td><a href='confirmation.php? adm_no=".$row['Admission_number']."'>
        <button class=\"btn btn-warning\"><span class=\"glyphicon glyphicon-edit\">Confirm</span></button>
        </a>
        <a href='sponsorreject.php? adm_no=".$row['Admission_number']."'><br><br>
        <button class=\"btn btn-danger\"><span class=\"glyphicon glyphicon-delete\">Reject</span></button></a>
        <a href='sponsorview2.php? adm_no=".$row['Admission_number']."'><br><br>
        <button class=\"btn btn-success\"><span class=\"glyphicon glyphicon-edit\">View Sponsor</span></button>
        </a>
        </td></tr>";
    }
    echo "</table>";
    ...
    <br>
    <hr />
                  <legend> <h3 align="center"> <b>CONFIRMED SPONSORED CHILDREN </b></h3></legend>
    <?php
...
$homeid=$_SESSION['id'];
    ...
    $rus = "SELECT child.Image,child.First_name,child.Last_name,child.Admission_number FROM child INNER JOIN confirmedspo WHERE child.Home_ID='$homeid' AND child.Admission_number=confirmedspo.Admission_number   ";
    $sur = $conn->Query($rus);
    ...
    while($row =mysqli_fetch_assoc($sur)) {
        ...
        $gi="<img src=".$ig." style='width:150px;height:150px;''>  <br>";
        echo "<tr><td> $gi </td><td>". $row["First_name"] . "</td><td>". $row["Last_name"]."</td><td>".$row["Admission_number"]."</td>";
        echo "<td><a href='sponsorview.php? adm_no=".$row['Admission_number']."'>
        <button class=\"btn btn-success\"><span class=\"glyphicon glyphicon-edit\">View Sponsor</span></button>
        </a>
        </td></tr>";
        }
