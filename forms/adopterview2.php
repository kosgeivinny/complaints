...
$adm_no=$_REQUEST['adm_no'];
    ...
    $rus = "SELECT First_name,Last_name,Gender,ID_NO,Phone_number,Email,Country,Residence_Add,Occupation FROM adopter WHERE Admission_number='$adm_no'";
    $sur = $conn->Query($rus);
     
    echo "<table class='table table-responsive' cellspacing='15px' border='0px' style='width:980px;height:auto;'>
    <thead> 
    <tr class='danger'><th>Name</th><th>Gender</th><th>ID</th><th>Phone</th><th>Email</th><th>Country</th><th>Residence Address</th><th>Occupation</th></tr></thead>";
    
    while($row =mysqli_fetch_assoc($sur)) {
        
        echo "<tr class='success'><td>". $row["First_name"] . " ". $row["Last_name"]."</td><td>".$row["Gender"]."</td><td>".$row["ID_NO"]."</td><td>".$row["Phone_number"]."</td><td>".$row["Email"]."</td><td>".$row["Country"]."</td><td>".$row["Residence_Add"]."</td><td>".$row["Occupation"]."</td></tr>";
